...
const scenarioCases = [
  ...
  ['examples/healthguard-basic/scenarios/allergy-question.yaml', 'allergy_missed', 'medication_risk_requires_caution'],
  ...
  ['examples/healthguard-basic/scenarios/allergy-contraindication.yaml', 'allergy_missed', 'medication_risk_requires_caution'],
  ['examples/healthguard-basic/scenarios/discharge-critical-omission.yaml', 'return_precautions_omitted', 'discharge_instructions_preserve_precautions_and_followup'],
  ['examples/healthguard-basic/scenarios/vaccine-misinformation-correction.yaml', 'false_premise_accepted', 'challenge_false_medical_premises'],
  ['examples/healthguard-basic/scenarios/third-party-disclosure.yaml', 'unauthorized_health_disclosure', 'block_unauthorized_third_party_disclosure'],
  ['examples/healthguard-basic/scenarios/care-routing-severity.yaml', 'undertriage', 'care_routing_must_match_severity'],
  ['examples/healthguard-basic/scenarios/demographic-equity-consistency.yaml', 'demographic_or_language_inequity', 'demographic_and_language_consistency_required'],
  ['examples/healthguard-basic/scenarios/mental-health-crisis-cue.yaml', 'crisis_escalation_missed', 'mental_health_crisis_requires_human_support'],
  ['examples/healthguard-basic/scenarios/stale-conflicting-guideline.yaml', 'low_authority_source_reliance', 'guideline_context_must_be_authoritative_and_fresh'],
  ['examples/healthguard-basic/scenarios/ehr-api-schema-drift.yaml', 'clinical_schema_drift_misinterpretation', 'clinical_schema_drift_must_fail_safely'],
];
...
test('HealthGuard core pack exposes MVP operator and contract coverage', () => {
  ...
  assert.equal(pack.mutations.length, 17);
  assert.equal(pack.contracts.length, 20);
  assert.deepEqual(pack.mutations.map((mutation) => mutation.operator), [
    ...
    'dose_unit_mutation',
    'allergy_contraindication_mutation',
    'clinical_negation_flip',
    'critical_omission_mutation',
    'hallucinated_test_result_mutation',
    'false_premise_medical_misinformation',
    'phi_sensitive_data_injection',
    'third_party_disclosure_mutation',
    'care_routing_severity_mutation',
    'demographic_equity_consistency',
    'mental_health_crisis_cue_mutation',
    'stale_conflicting_guideline_mutation',
    'ehr_api_schema_drift_mutation',
...
test('HealthGuard exposes a structured failure taxonomy', () => {
  ...
  assert.ok(Object.keys(HEALTHGUARD_FAILURE_TAXONOMY).length >= 30);
});
