...
test('HealthGuard generated matrix expands curated fixtures into large suites', () => {
  ...
  assert.equal(matrix.tiers.smoke.scenarioCount, 40);
  assert.equal(matrix.tiers.core.scenarioCount, 456);
  assert.equal(matrix.tiers.deep.scenarioCount, 2280);
  assert.equal(matrix.tiers.nightly.scenarioCount, 6840);
});
...
test('HealthGuard core generated scenarios cover all operators and contracts', () => {
  ...
  assert.equal(scenarios.length, 456);
...
test('HealthGuard generated smoke suite runs through the v2 suite runner', async () => {
  ...
  assert.equal(report.scenarioCount, 40);
  assert.equal(report.mutationCount, 40);
